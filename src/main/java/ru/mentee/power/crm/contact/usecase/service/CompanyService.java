...
@AllArgsConstructor
public class CompanyService implements CreateCompanyUseCase, GetCompanyUseCase, ListCompaniesUseCase {
    ...
    public Company create(String name, List<PersonLinkCommand> personLinks) {
        ...
        List<PersonCompanyLink> links = Optional.ofNullable(personLinks)
                .stream()
                .flatMap(List::stream)
    ...
    private PersonCompanyLink toPersonCompanyLink(PersonLinkCommand command) {
        ...
        personRepository.findById(command.personId())
